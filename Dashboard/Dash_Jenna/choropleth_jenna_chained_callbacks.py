...
#                     )
states_dropdown = dcc.Dropdown(options=[{'label': s, 'value': s} for s in sorted(df.state_id.unique())],
                        value='AK',
                        clearable=False)
counties_dropdown = dcc.Dropdown(options=[],multi=False,clearable=False)
)
#--------------------------------------------------------------------------
...
app.layout = dbc.Container([
    dbc.Row([
        dbc.Col([states_dropdown], width=6)
    ], justify='center'),
    dbc.Row([
        dbc.Col([counties_dropdown], width=6)
    ], justify='center')
    dbc.Row([
