...
4. Supervised ML Binary Classification
   - The benefit of choosing a binary classifier model is that there are many models to choose from and many hyperparameters for tuning. The drawback is that we are limited to priorityrget values (high-prority to low-priority). 
